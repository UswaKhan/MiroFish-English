...
def get_generate_status():
    ...
    try:
        ...
        # If simulation_id provided, check if completed report exists
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify(
...
def download_report(report_id: str):
    ...
    try:
        ...
        if not os.path.exists(md_path):
            # Generate temp file if MD doesn't exist
            import tempfile
...
def get_agent_log(report_id: str):
    """
    Get detailed execution logs from Report Agent
    ...
    Query parameters:
        from_line: start from line number (optional, default 0, for incremental fetch)
...
def get_console_log(report_id: str):
    """
    Get console output logs from Report Agent

    Real-time access to console output during report generation (INFO, WARNING, etc.),
    different from agent-log endpoint which returns structured JSON logs,
