...
def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize interview prompts by adding prefix to prevent Agent from calling tools
...
def get_graph_entities(graph_id: str):
    ...
    Only returns nodes that match predefined entity types (not just Entity Labels)
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # Detailed log
        logger.debug(
            f"Checking simulation prep status: {simulation_id}, status={status}, config_generated={config_generated}"
        )

        # If config_generated=True and files exist, consider it prepared
        # The following statuses indicate prep work is complete:
        # - ready: prepared, can run
        # - preparing: if config_generated=True, means completed
        # - running: running, means prep was complete long ago
        # - completed: completed, means prep was complete long ago
        # - stopped: stopped, means prep was complete long ago
        # - failed: failed (but prep is complete)
        prepared_statuses = [
            "ready",
            "preparing",
            "running",
        ...
        if status in prepared_statuses and config_generated:
            ...
            if status == "preparing":
                try:
                    ...
                    logger.info(
                        f"Auto-update simulation status: {simulation_id} preparing -> ready"
                    )
                    status = "ready"
                except Exception as e:
            ...
            logger.info(
                f"Simulation {simulation_id} check result: prepared (status={status}, config_generated={config_generated})"
            )
            return True, {
                "status": status,
            ...
            logger.warning(
                f"Simulation {simulation_id} check result: not prepared (status={status}, config_generated={config_generated})"
            )
            return False, {
...
def prepare_simulation():
    ...
    Features:
    - Auto-detect completed prep work, avoid duplicate generation
    - If already prepared, return existing result directly
    - Support force regenerate (force_regenerate=true)

    Steps:
    1. Check if prep work is already completed
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Prep task started|existing prep work completed",
    ...
    try:
        ...
        if not force_regenerate:
            ...
            if is_prepared:
                ...
                return jsonify(
                    {
                        ...
                        "data": {
                            ...
                            "status": "ready",
                            "message": "Existing prep work completed, no need to regenerate",
                ...
                logger.info(
                    f"Simulation {simulation_id} not prepared, will start prep task"
                )
        ...
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False,  # No edge info to speed up
            )
            ...
            logger.warning(
                f"Failed to sync entity count (will retry in background task): {e}"
            )
        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    "task_id": task_id,
                    "status": "preparing",
                    "message": "Prep task started, query progress via /api/simulation/prepare/status",
...
def get_prepare_status():
    """
    Query prep task progress

    Supports two query methods:
    1. Query ongoing task progress via task_id
    2. Check if prep work is already completed via simulation_id
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "progress": 45,
                "message": "...",
                "already_prepared": true|false,  // whether has completed prep
    ...
    try:
        ...
        # If simulation_id provided, check if prep is already complete
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify(
                    {
                        ...
                        "data": {
                            ...
                            "status": "ready",
                            "progress": 100,
                            "message": "Existing prep work completed",
                            "already_prepared": True,
                            "prepare_info": prepare_info,
                        },
                    }
                ...
                return jsonify(
                    {
                        ...
                        "data": {
                            ...
                            "status": "not_started",
                            "progress": 0,
                            "message": "Prep not started yet, call /api/simulation/prepare to start",
                            "already_prepared": False,
                        },
                    }
                )
        ...
        if not task:
            # Task doesn't exist, but if has simulation_id, check if prep is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify(
                        {
                            ...
                            "data": {
                                ...
                                "status": "ready",
                                "progress": 100,
                                "message": "Task completed (prep work existed)",
                                "already_prepared": True,
                                "prepare_info": prepare_info,
                            },
                        }
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    """
    Get latest report_id corresponding to simulation
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get simulation's Agent Profiles in real-time (for viewing progress during generation)
    ...
    - Returns extra metadata (like file modification time, whether generating, etc.)
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns extra metadata (like file modification time, whether generating, etc.)
    - Can return partial info even if config is not yet complete
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation config (LLM intelligently generated complete config)
...
def download_simulation_script(script_name: str):
    ...
    try:
        ...
        if not os.path.exists(script_path):
            return jsonify(
                {"success": False, "error": f"Script file not found: {script_name}"}
            ), 404
...
def start_simulation():
    ...
    About force parameter:
        - When enabled, if simulation is running or completed, will first stop and clean run logs
        ...
        - Suitable for scenarios requiring rerun
    ...
    try:
        ...
        # Smart status handling: if prep work is complete, allow restart
        if state.status != SimulationStatus.READY:
            # Check if prep work is complete
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)

            if is_prepared:
                # Prep work is complete, check if there's a running process
                ...
                logger.info(
                    f"Simulation {simulation_id} prep work is complete, resetting status to ready (original status: {state.status.value})"
                )
        ...
        # Get graph_id (for graph memory update)
        graph_id = None
...
def interview_agent():
    ...
    Note: This feature requires simulation environment to be running (enter command-waiting mode after simulation loop completes)
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // optional, specify platform (twitter/reddit)
                                               // not specified: dual-platform simulation interviews both platforms simultaneously
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify(
                {
                    "success": False,
                    "error": "Simulation environment not running or closed. Please ensure simulation is completed and entered command-waiting mode.",
                }
            ), 400
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What's your view on B?"  // no platform means use default
                }
            ],
            "platform": "reddit",              // optional, default platform (overridden by each item's platform)
                                               // not specified: dual-platform simulation interviews both platforms for each Agent
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // required, simulation ID
            "prompt": "What's your overall view on this?",  // required, interview question (all Agents use same question)
...
def get_interview_history():
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",          // optional, platform type (reddit/twitter)
                                           // not specified returns all history from both platforms
    ...
    try:
        ...
        simulation_id = data.get("simulation_id")
        platform = data.get("platform")  # not specified returns both platforms' history
        agent_id = data.get("agent_id")
