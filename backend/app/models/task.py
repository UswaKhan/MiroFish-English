...
Used for tracking long-running tasks (like graph building)
"""
...
class TaskManager:
    ...
    def create_task(self, task_type: str, metadata: Optional[Dict] = None) -> str:
        ...
        with self._task_lock:  # type: ignore
            self._tasks[task_id] = task  # type: ignore
    ...
    def get_task(self, task_id: str) -> Optional[Task]:
        """Get task"""
        with self._task_lock:  # type: ignore
            return self._tasks.get(task_id)  # type: ignore
    ...
    def list_tasks(self, task_type: Optional[str] = None) -> list:
        """List tasks"""
        with self._task_lock:  # type: ignore
            tasks = list(self._tasks.values())  # type: ignore
            if task_type:
    ...
    def cleanup_old_tasks(self, max_age_hours: int = 24):
        ...
        with self._task_lock:  # type: ignore
            old_ids = [
                tid
                for tid, task in self._tasks.items()  # type: ignore
                if task.created_at < cutoff
                and task.status in [TaskStatus.COMPLETED, TaskStatus.FAILED]
            ]
            for tid in old_ids:
                del self._tasks[tid]  # type: ignore
