...
Graph search, node reading, edge querying services for Report Agent use
...
class InterviewResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            ...
            f"**Number of Agents Interviewed:** {self.interviewed_count} / {self.total_agents}",
    ...
    def insight_forge(
        ...
        for uuid in list(entity_uuids):  # Process all entities, no truncation
            if not uuid:
                continue
            try:
                # Get each related node's info separately
...
Return the sub-questions in JSON format."""
    ...
    def interview_agents(
        ...
        if not profiles:
            logger.warning(
                f"Could not find persona files for simulation {simulation_id}"
            )
            result.summary = "Could not find interviewable Agent persona files"
            return result
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "based on your persona, all your past memories and actions.\n"
            ...
            "4. Answer each question one by one by number, start each answer with 'Question X:'\n"
            "5. Separate each answer with an empty line\n"
            "6. Answers should have substantial content, each question should be answered in at least 2-3 sentences\n\n"
        )
        optimized_prompt = f"{INTERVIEW_PROMPT_PREFIX}{combined_prompt}"

        # Step 4: Call the real interview API (don't specify platform, dual-platform interview by default)
        try:
            # Build batch interview list (don't specify platform, API interviews on both twitter and reddit)
            interviews_request = []
                ...
                interviews_request.append(
                    {
                        ...
                        # Don't specify platform, API will interview on both twitter and reddit
                    }
                )
            ...
            # Call SimulationRunner's batch interview method (don't pass platform, dual-platform interview)
            api_result = SimulationRunner.interview_agents_batch(
                simulation_id=simulation_id,
                interviews=interviews_request,
                platform=None,  # Don't specify platform, dual-platform interview
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Strategy 1 (main): Extract complete substantive sentences
                ...
                # Strategy 2 (supplement): Long text inside correctly paired Chinese quotes 「」
                if not key_quotes:
                    paired = re.findall(
                        r"\u201c([^\u201c\u201d]{15,100})\u201d", clean_text
                    )
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        ...
        # Priority: try to read Reddit JSON format
        ...
        if os.path.exists(reddit_profile_path):
            try:
                ...
                logger.info(
                    f"Loaded {len(profiles)} personas from reddit_profiles.json"
                )
                return profiles
            except Exception as e:
        ...
        if os.path.exists(twitter_profile_path):
            try:
                ...
                logger.info(
                    f"Loaded {len(profiles)} personas from twitter_profiles.csv"
                )
                return profiles
            except Exception as e:
    ...
    def _select_agents_for_interview(
        ...
        Returns:
            tuple: (selected_agents, selected_indices, reasoning)
                - selected_agents: List of selected Agents' complete information
                - selected_indices: List of selected Agents' indices (for API calls)
        ...
        system_prompt = """You are a professional interview planning expert. Your task is to select the most suitable Agents to interview from the simulation Agent list based on interview requirements.
...
2. Different answers may be expected from different roles
3. Cover multiple dimensions: facts, opinions, feelings, etc.
4. Language should be natural, like a real interview
5. Each question should be within 50 characters, concise and clear
...
Please generate 3-5 interview questions."""
        ...
        try:
            ...
            return response.get(
                "questions", [f"What is your opinion about {interview_requirement}?"]
            )

        except Exception as e:
            logger.warning(f"Failed to generate interview questions: {e}")
            return [
                f"What are your views on {interview_requirement}?",
...
- Use plain text paragraphs, separate different sections with empty lines
- Do not use Markdown headings (like #, ##, ###)
- Do not use horizontal rules (like ---, ***)
- Use "「」" quotes when quoting interviewees' original words
...
Please generate the interview summary."""
        ...
        try:
            ...
            return (
                f"Interviewed {len(interviews)} interviewees, including: "
