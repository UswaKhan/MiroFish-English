...
Endpoint 1: Analyze text content, generate entity and relation type definitions suitable for social simulation
"""
...
class OntologyGenerator:
    ...
    # Maximum text length passed to LLM (50,000 chars)
    ...
    def _build_user_message(
        ...
        # If text exceeds 50,000 chars, truncate (only affects content passed to LLM, not graph building)
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            combined_text = combined_text[: self.MAX_TEXT_LENGTH_FOR_LLM]
            combined_text += f"\n\n...(original has {original_length} chars, truncated to first {self.MAX_TEXT_LENGTH_FOR_LLM} chars for ontology analysis)..."
