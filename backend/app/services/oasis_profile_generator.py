...
3. Differentiate individual entities and abstract group entities
"""
...
class OasisProfileGenerator:
    ...
    Converts entities from Zep graph to Agent Profile required for OASIS simulation
    ...
    def __init__(
        ...
        # Zep client for retrieval and context enrichment
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep doesn't have built-in hybrid search, need to search edges and nodes separately then merge.
        ...
        try:
            ...
            logger.info(
                f"Zep hybrid search complete: {entity_name}, got {len(results['facts'])} facts, {len(results['node_summaries'])} related nodes"
            )
    ...
    def _is_group_entity(self, entity_type: str) -> bool:
        """Check if it's a group/institution type entity"""
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    temperature=0.7
                    - (attempt * 0.1),  # Decrease temperature with each retry
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user persona generation expert. Generate detailed, realistic personas for public opinion simulation, maximizing restoration of existing real-world situations. Must return valid JSON format, all string values must not contain unescaped newlines. Use English."
        return base_prompt
...
2. persona: Detailed persona description (2000 characters of plain text), must include:
   ...
   - Stance and views (attitude towards the topic, content that may anger/move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memories (important part of the persona, describing this individual's connection to the event and their existing actions/reactions in the event)
3. age: Age (must be an integer)
4. gender: Gender, must be English: "male" or "female"
5. mbti: MBTI type (e.g., INTJ, ENFP, etc.)
6. country: Country (use English, e.g., "China", "USA", "UK")
...
- Use English for all content (except gender field which must be English: male/female)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account persona for the institution/group entity, maximizing restoration of existing real-world situations.
...
2. persona: Detailed account persona description (2000 characters of plain text), must include:
   ...
   - Stance and attitude (official position on core topics, handling methods for controversies)
   - Special notes (represented group profile, operational habits)
   - Institution memory (important part of the persona, describing this institution's connection to the event and their existing actions/reactions in the event)
3. age: Always set to 30 (virtual age for institutional accounts)
4. gender: Always set to "other" (institutional accounts use other to indicate non-personal)
...
- Use English for all content (except gender field which must be English: "other")
- age must be integer 30, gender must be string "other"
- Institutional accounts must speak in accordance with their identity positioning
"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            realtime_output_path: Real-time write file path (if provided, write each time one is generated)
        ...
        print(f"\n{'=' * 60}")
        print(
            f"Starting Agent persona generation - {total} entities, parallel count: {parallel_count}"
        )
        print(f"{'=' * 60}\n")
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    logger.error(
                        f"Exception when processing entity {entity.name}: {str(e)}"
                    )
                    with lock:
                        completed_count[0] += 1
    ...
    def _print_generated_profile(
        ...
        topics_str = (
            ", ".join(profile.interested_topics) if profile.interested_topics else "N/A"
        )
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profile in CSV format (conforms to OASIS official requirements)
        ...
        with open(file_path, "w", newline="", encoding="utf-8") as f:
            ...
            for idx, profile in enumerate(profiles):
                ...
                # Handle newlines (use space in CSV)
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        for idx, profile in enumerate(profiles):
            ...
            item = {
                ...
                else idx,  # Key: must include user_id
