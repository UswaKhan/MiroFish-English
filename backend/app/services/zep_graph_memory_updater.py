...
class ZepGraphMemoryUpdater:
    ...
    All meaningful behaviors are updated to Zep, action_args contains complete context:
    ...
    # Batch size (how many activities per platform before sending)
    BATCH_SIZE = 5
    ...
    # Send interval in seconds, to avoid too fast requests
    SEND_INTERVAL = 0.5
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args contains complete context (e.g., post content, usernames).
        ...
        with self._buffer_lock:
            for platform, buffer in self._platform_buffers.items():
                if buffer:
                    display_name = self._get_platform_display_name(platform)
                    logger.info(
                        f"Sending remaining {len(buffer)} {display_name} activities"
                    )
