...
- After simulation completes, don't close environment immediately, enter command wait mode
...
# Load .env file from project root (contains LLM_API_KEY etc config)
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai warnings about max_tokens (we intentionally don't set max_tokens, let the model decide)"""
...
# Add filter immediately on module load, ensure it takes effect before camel code executes
...
def init_logging_for_simulation(simulation_dir: str):
    """
    Initialize simulation's logging config
...
class ParallelIPCHandler:
    ...
    async def handle_interview(
        ...
        # Parallel interview of both platforms
        tasks = []
...
# Non-core action types to filter out (these actions have low value for analysis)
...
def fetch_new_actions_from_db(
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, only keep key fields (keep complete content, don't truncate)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    try:
        ...
        # Post comment: supplement post info being commented on
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If boost LLM is configured, during parallel simulation different platforms can use different API providers to improve concurrency.
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError(
            "Missing API Key config, please set LLM_API_KEY in .env file in project root"
        )
...
async def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Exit immediately after simulation completes, do not enter command wait mode",
    )
    ...
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering command wait mode - environment kept running")
...
def setup_signal_handlers(loop=None):
    ...
    def signal_handler(signum, frame):
        ...
        # Do not directly sys.exit(), allow asyncio loop to exit normally and clean up resources
