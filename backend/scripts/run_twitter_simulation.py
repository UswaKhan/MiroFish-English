...
Features:
- After simulation completes, do not immediately close environment, enter command wait mode
...
class TwitterSimulationRunner:
    ...
    async def run(self, max_rounds: int = None):
        ...
        if self.wait_for_commands:
            print("\n" + "=" * 60)
            print("Entering command wait mode - environment kept running")
...
def setup_signal_handlers():
    ...
    Allow program to properly clean up resources (close database, environment, etc.)
    """
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            ...
            # Force exit only when signal received again
            print("Force exit...")
            sys.exit(1)
